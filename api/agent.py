    ...
    },
    {
        ...
        "function": {
            ...
            "parameters": {
                ...
                "required": ["leave_record_id", "employee_email"],
            },
        },
    },
]
...
def _call_llm(llm_client, model_name: str, messages: list, use_tools: bool = True):
    ...
    try:
        ...
        err_str = str(e)
        # Groq tool_use_failed — retry with tools at lower tokens first
        if "tool_use_failed" in err_str or ("400" in err_str and "tool" in err_str.lower()):
            logger.warning(f"[Agent] tool_use_failed — retrying with tools at lower tokens")
            try:
                ...
                logger.warning(f"[Agent] Retry also failed — falling back without tools")
                return llm_client.chat.completions.create(
                    model       = model_name,
                    messages    = messages,
                    max_tokens  = 512,
            ...
            async with ClientSession(mcp_read, mcp_write) as mcp_session:
                ...
                while iteration < max_iterations:
                    ...
                    for tc in tool_calls:
                        ...
                        # ── Always inject employee_email for get_employee_record and cancel_leave ─
                        if tool_name in ("get_employee_record", "cancel_leave"):
                        ...
                        if tool_name == "apply_leave":
                            ...
                            if not balance_fetched:
                                ...
                                try:
                                    ...
                                    balance_list = bal_data.get("leave_balance_list", [])

                                    # Get leave type requested by user from conversation
                                    user_text = " ".join([
                                    ...
                                    # Try to match by leave type name mentioned in conversation
                                    for lt in balance_list:
                                        lt_name = lt["leave_type_name"].lower()
                                        if lt.get("leave_type_type") == "ABSENT":
                                            continue
                                        if "privilege" in user_text and "privilege" in lt_name:
                                            matched_id = lt["leave_type_id"]
                                            matched_name = lt["leave_type_name"]
                                            break
                                        elif ("sick" in user_text or "casual" in user_text) and ("sick" in lt_name or "casual" in lt_name):
                                            matched_id = lt["leave_type_id"]
                                            matched_name = lt["leave_type_name"]
                                            break

                                    # Fallback: pick first non-absent leave type
                                    if not matched_id:
                                        for lt in balance_list:
                                            if lt.get("leave_type_type") != "ABSENT":
                                                matched_id = lt["leave_type_id"]
                                    ...
                                    if matched_id:
                                        tool_args["leave_type_id"] = matched_id
                                        logger.info(f"[Agent] Corrected leave_type_id={matched_id} ({matched_name})")
                        ...
                        # ── Force get_leave_records before cancel_leave ───────
                        if tool_name == "cancel_leave":
                            records_fetched = any(
                                "leave_records" in msg.get("content", "")
                                for msg in all_messages
                                if msg.get("role") == "tool"
                            )
                            if not records_fetched:
                                logger.info("[Agent] Forcing get_leave_records before cancel_leave")
                                rec_args = {
                                    "access_token":     access_token,
                                    "employee_zoho_id": tool_args.get("employee_zoho_id") or str(employee_info.get("id", "")),
                                }
                                try:
                                    rec_result = await mcp_session.call_tool("get_leave_records", rec_args)
                                    rec_text = rec_result.content[0].text if rec_result and rec_result.content else "{}"
                                    logger.info(f"[Agent] Force-fetched leave records: {rec_text[:300]}")
                                    all_messages.append({
                                        "role": "tool",
                                        "tool_call_id": "force_records",
                                        "content": rec_text,
                                    })
                                    # Match leave_record_id from records by date if not provided
                                    if not tool_args.get("leave_record_id"):
                                        rec_data = json.loads(rec_text)
                                        user_text = " ".join([
                                            m.get("content", "") for m in all_messages
                                            if m.get("role") == "user"
                                        ]).lower()
                                        for rec in rec_data.get("leave_records", []):
                                            from_date = rec.get("leave_from_date", "").lower()
                                            to_date   = rec.get("leave_to_date",   "").lower()
                                            if from_date in user_text or to_date in user_text:
                                                tool_args["leave_record_id"] = rec["leave_record_id"]
                                                logger.info(f"[Agent] Auto-matched leave_record_id={rec['leave_record_id']} for date {from_date}")
                                                break
                                except Exception as re:
                                    logger.error(f"[Agent] Force records fetch error: {re}")
                        ...
                        logger.info(f"[Agent] Tool result: {result_text}")
